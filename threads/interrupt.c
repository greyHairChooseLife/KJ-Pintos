...
/* FUNCTION을 호출하는 게이트(gate)를 생성합니다.
   ...
   이 게이트는 디스크립터 권한 레벨(DPL)을 가집니다. 즉,
   프로세서가 DPL 또는 그보다 낮은 번호의 링(ring)에 있을 때
   의도적으로 호출될 수 있습니다. 실제로는, DPL==3은 사용자 모드가
   게이트를 호출할 수 있게 허용하고 DPL==0은 그러한 호출을 막습니다.
   사용자 모드에서 발생하는 폴트(fault)와 예외(exception)는
   여전히 DPL==0인 게이트가 호출되도록 합니다.

   TYPE은 반드시 14 (인터럽트 게이트) 또는 15 (트랩 게이트)여야 합니다.
   차이점은 인터럽트 게이트로 진입하면 인터럽트가 비활성화되지만,
   트랩 게이트로 진입하면 그렇지 않다는 것입니다.
   [IA32-v3a] 섹션 5.12.1.2 "Flag Usage By Exception- or
   Interrupt-Handler Procedure"에서 논의를 참조하세요. */

struct gate {
    unsigned off_15_0 : 16;   // 세그먼트 내 오프셋의 하위 16비트
    unsigned ss : 16;         // 세그먼트 셀렉터
    unsigned ist : 3;         // 인자 수(# args), 인터럽트/트랩 게이트는 0
    unsigned rsv1 : 5;        // 예약됨 (0이어야 함)
    unsigned type : 4;        // 타입 (STS_{TG,IG32,TG32})
    unsigned s : 1;           // 0이어야 함 (시스템)
    unsigned dpl : 2;         // 디스크립터 (즉, 새로운) 권한 레벨
    unsigned p : 1;           // 존재(Present) 비트
    unsigned off_31_16 : 16;  // 세그먼트 내 오프셋의 상위 비트
    uint32_t off_32_63;
    uint32_t rsv2;
};

/* 인터럽트 디스크립터 테이블 (IDT). 이 형식은 CPU에 의해 고정되어 있습니다.
   [IA32-v3a] 섹션 5.10 "Interrupt Descriptor
   Table (IDT)", 5.11 "IDT Descriptors", 5.12.1.2 "Flag Usage By
   Exception- or Interrupt-Handler Procedure"를 참조하세요. */
...
/* 각 인터럽트를 위한 인터럽트 핸들러 함수들. */
static intr_handler_func *intr_handlers[INTR_CNT];

/* 디버깅 목적의 각 인터럽트 이름들. */
static const char *intr_names[INTR_CNT];

/* 외부 인터럽트(External interrupts)는 타이머와 같이 CPU 외부의
   장치에 의해 생성되는 것들입니다. 외부 인터럽트는
   인터럽트가 꺼진(off) 상태로 실행되므로, 절대 중첩되지 않으며
   선점(pre-empted)되지도 않습니다. 핸들러는 또한
   잠들 수(sleep) 없지만, intr_yield_on_return()을 호출하여
   인터럽트 반환 직전에 새 프로세스가 스케줄되도록
   요청할 수 있습니다. */
static bool in_external_intr;   /* 현재 외부 인터럽트를 처리 중인가? */
static bool yield_on_return;    /* 인터럽트 반환 시 양보(yield)해야 하는가? */

/* 프로그래머블 인터럽트 컨트롤러(PIC) 헬퍼 함수들. */
...
intr_get_level (void) {
    ...
    /* 플래그 레지스터를 프로세서 스택에 푸시(push)한 다음,
       스택에서 팝(pop)하여 그 값을 `flags`에 저장합니다.
       [IA32-v2b] "PUSHF"와 "POP", 그리고 [IA32-v3a] 5.8.1 "Masking Maskable Hardware
       Interrupts"를 참조하세요. */
...
/* 인터럽트 VEC_NO를 등록하여 HANDLER가 디스크립터 권한 레벨
...
/* 내부 인터럽트 VEC_NO를 등록하여 HANDLER를 호출하도록 합니다.
   디버깅 목적으로 NAME이 이름으로 사용됩니다. 인터럽트 핸들러는
   인터럽트 상태 LEVEL에서 호출됩니다.

   핸들러는 디스크립터 권한 레벨 DPL을 가집니다. 즉,
   프로세서가 DPL 또는 그보다 낮은 번호의 링(ring)에 있을 때
   의도적으로 호출될 수 있습니다. 실제로는, DPL==3은 사용자 모드가
   인터럽트를 호출할 수 있게 허용하고 DPL==0은 그러한
   호출을 막습니다. 사용자 모드에서 발생하는 폴트와 예외는
   여전히 DPL==0인 인터럽트가 호출되도록 합니다.
...
/* 모든 PC에는 두 개의 8259A 프로그래머블 인터럽트 컨트롤러 (PIC)
   칩이 있습니다. 하나는 포트 0x20과 0x21로 접근 가능한 "마스터"입니다.
   다른 하나는 마스터의 IRQ 2 라인에 캐스케이드(연결)된 "슬레이브"이며
   포트 0xa0과 0xa1로 접근 가능합니다. 포트 0x20에 대한 접근은
   A0 라인을 0으로 설정하고 0x21에 대한 접근은 A1 라인을 1로
   설정합니다. 슬레이브 PIC의 경우도 상황은 비슷합니다.

   기본적으로, PIC에 의해 전달되는 0...15번 인터럽트는
   인터럽트 벡터 0...15로 갑니다. 불행히도, 이 벡터들은
   CPU 트랩과 예외에도 사용됩니다. 우리는 PIC를
   재프로그래밍하여 0...15번 인터럽트가 대신 인터럽트 벡터
...
pic_init (void) {
    ...
    /* 마스터 초기화. */
    outb (0x20, 0x11); /* ICW1: 싱글 모드, 엣지 트리거, ICW4 필요. */
    outb (0x21, 0x20); /* ICW2: IR0...7 라인 -> irq 0x20...0x27. */
    outb (0x21, 0x04); /* ICW3: IR2 라인에 슬레이브 PIC 연결. */
    outb (0x21, 0x01); /* ICW4: 8086 모드, 일반 EOI, 비-버퍼 모드. */

    /* 슬레이브 초기화. */
    outb (0xa0, 0x11); /* ICW1: 싱글 모드, 엣지 트리거, ICW4 필요. */
    outb (0xa1, 0x28); /* ICW2: IR0...7 라인 -> irq 0x28...0x2f. */
    outb (0xa1, 0x02); /* ICW3: 슬레이브 ID는 2. */
    outb (0xa1, 0x01); /* ICW4: 8086 모드, 일반 EOI, 비-버퍼 모드. */

    /* 모든 인터럽트를 언마스크(차단 해제)합니다. */
    outb (0x21, 0x00);
    outb (0xa1, 0x00);
}
...
pic_end_of_interrupt (int irq) {
    ...
    /* 슬레이브 인터럽트인 경우 슬레이브 PIC에도 확인 신호. */
    if (irq >= 0x28)
        outb (0xa0, 0x20);
}
/* 인터럽트 핸들러. */

/* 모든 인터럽트, 폴트, 예외를 위한 핸들러입니다. 이
...
intr_handler (struct intr_frame *frame) {
    ...
    /* 외부 인터럽트는 특별합니다.
       우리는 한 번에 하나만 처리하며(따라서 인터럽트는 off여야 함)
       PIC에 확인 신호(acknowledge)를 보내야 합니다 (아래 참조).
       외부 인터럽트 핸들러는 잠들(sleep) 수 없습니다. */
    ...
    if (handler != NULL)
        ...
        /* 핸들러가 없지만, 이 인터럽트는 하드웨어 결함이나
           하드웨어 경쟁 조건으로 인해 가짜로(spuriously)
           발생할 수 있습니다. 무시합니다. */
    } else {
        /* 핸들러가 없고 가짜 인터럽트도 아님. 예상치 못한
           인터럽트 핸들러를 호출합니다. */
